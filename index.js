...
async function run() {
  try {
    ...
    const coffeeCollection = client.db('coffeeDB').collection('coffes');
    ...
    app.put('/coffes/:id', async(req, res) => {
        ...
        const result = await coffeeCollection.updateOne(query, updatedCoffee, option);
        res.send(result);
    });
...
app.get('/', (req, res) => {
  res.send('Espresso Coffee is processing!');
});
